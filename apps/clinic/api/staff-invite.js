...
const ALLOWED_ROLES = new Set(['doctor', 'receptionist', 'assistant', 'admin'])
...
export default async function handler(req, res) {
  ...
  if (!ALLOWED_ROLES.has(normalizedRole)) return res.status(400).json({ error: 'Invalid staff role' })
  ...
  const rec = { email, full_name, role: normalizedRole, user_id: invited.user.id, active: true }
  const { error: staffErr } = existing
    ? await admin.from('staff').update(rec).eq('id', existing.id)
    : await admin.from('staff').insert(rec)
  if (staffErr) {
    // Do not leave behind an invited Auth user that has no staff record.
    await admin.auth.admin.deleteUser(invited.user.id)
    return res.status(502).json({ error: staffErr.message })
  }
